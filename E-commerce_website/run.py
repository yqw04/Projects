...
# Table to link users and their baskets
...
class Checkout(FlaskForm):
    ...
    card_no = StringField("Card Number", validators = [Length(16, 16), DataRequired()])
    cvc = StringField("CVC", validators = [Length(3, 3), DataRequired()]) # 2 no. in Length - min and max
...
# Needed when working with users
@lm.user_loader
def load_user(user_id):
    return User.query.get(int(user_id))

# Just '/' as its the first page users see
@app.route('/', methods = ['GET', 'POST']) # Get data | send data to server
def home():
    form = AddBasket()
    new_arrivals = Item.query.filter(Item.id.in_([7, 8, 9])).all()  # Specific IDs shown for the homepage sections
    popular = Item.query.filter(Item.id.in_([10, 11, 12])).all() 
...
def signup():
    ...
    # Creates new user, adds to database
...
def shop():
    ...
    category = request.args.get('category', 'shop')
    ...
    # For sorting items into specific ways
    if category == 'more' :
    ...
    return render_template('shop.html', form = form, products = products, category_form = category_form, category = category, user = user)
...
def add_to_basket(item_id):
    ...
    # Checks if the item already exists in the basket
...
def update_quantity(item):
    ...
    if link:
        ...
        if quantity > 0:  # Ensure the quantity is positive
            ...
            # Handle the case where quantity is 0 or less, removes the item
...
def checkout():
    ...
    basket = Basket()
    total = price(basket)
    card_number = form.card_no.data # Data from checkout form
    cvc = form.cvc.data
    error = ''
    if form.validate_on_submit():
...
def finish_purchase():
    basket_items = UserBasketLink.query.filter_by(user_id = current_user.get_id()).all()
...
# All items sold on website, loaded when code runs
def insert_items():
